...
func (c *customerAccountDao) Check(q *bd.Query) (int, error) {
	var count int64
	err := c.Table(c.NewEntity().TableName()).Where(q.Filter).Count(&count).Error()
	if err != nil {
		return 0, err
	}
...
func (c *customerAccountDao) UpdateByID(i int, e entity.Entity) (entity.Entity, error) {
	err := c.Omit(clause.Associations).Where(&entity.CustomerAccount{ID: i}).Updates(e).Error()
	return e, err
}
