...
func (ud *userDao) Check(q *bd.Query) (int, error) {
	var count int64
	err := ud.Table(ud.NewEntity().TableName()).Where(q.Filter).Count(&count).Error()
	if err != nil {
		return 0, err
	}
...
func (ud *userDao) UpdateByID(i int, e entity.Entity) (entity.Entity, error) {
	err := ud.Omit(clause.Associations).Where(&entity.User{ID: i}).Updates(e).Error()
	return e, err
}
