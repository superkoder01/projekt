...
func (w *workerDao) Check(q *bd.Query) (int, error) {
	var count int64
	err := w.Table(w.NewEntity().TableName()).Where(q.Filter).Count(&count).Error()
	if err != nil {
		return 0, err
	}
...
func (w *workerDao) UpdateByID(i int, e entity.Entity) (entity.Entity, error) {
	err := w.Omit(clause.Associations).Where(&entity.Worker{ID: i}).Updates(e).Error()
	return e, err
}
