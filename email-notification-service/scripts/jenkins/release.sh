...
new_tag_value () {
  ...
  last_section=$(echo $tag_input | cut -d. -f3)
  ...
  case $release_type in
    minor)
      new_tag_value=$first_section.$middle_section.$(($last_section+1))
      ;;
    feature)
      new_tag_value=$first_section.$(($middle_section+1)).0
      ;;
    api)
      new_tag_value=v$(($first_index+1)).0.0
      ;;
    *)
...
replace_old_tag_with_new_tag () {
  ...
  DEFAULT_FILES_TO_REPLACE_OLD_TAG_INFO=( "./deployments/k8s/overlays/$2/*" "./scripts/jenkins/build_image.sh" )
